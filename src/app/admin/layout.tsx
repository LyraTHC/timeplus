...
export default function AdminLayout({
  ...
  useEffect(() => {
    if (isFirebaseConfigured && !loading) {
      if (!user) {
        // If not logged in, redirect to the secret login page
  ...
  const navLinks = [
    ...
    { href: "/admin/transactions", label: "Transações", icon: Wallet },
    { href: "/admin/payouts", label: "Retiradas", icon: HandCoins },
      ...
      <div className="hidden border-r bg-card md:block">
        <div className="flex h-full max-h-screen flex-col gap-2">
          <div className="flex h-20 items-center justify-center border-b px-4 lg:h-24 lg:px-6">
            <Link href="/admin" className="flex items-center gap-2 font-semibold w-full px-4">
              <Logo />
            </Link>
          </div>
          ...
          <Sheet>
            ...
            <SheetContent side="left" className="flex flex-col">
              <nav className="grid gap-2 text-lg font-medium">
                ...
                >
                  <Logo />
                  <span className="sr-only">TimePlus Admin</span>
                </Link>
                {navLinks.map((link) => (
                  <Link
                    key={link.href}
